  ...
  formatter({ on }) {
    ...
    on("message", (envelope) => {
      ...
      if (envelope.testCaseFinished) {
        const status = worstStatusByCase.get(envelope.testCaseFinished.testCaseStartedId) ?? "UNKNOWN";
        scenarioTotal += 1;
      ...
      if (envelope.testRunFinished) {
        ...
        const breakdown = Object.keys(scenarioCountByStatus)
          ...
          .map((status) => `${STATUS_COLOR[status] || C.gray}${scenarioCountByStatus[status]} ${status.toLowerCase()}${C.reset}`)
          .join(`${C.dim}, ${C.reset}`);

        console.log(`\n${C.bold}${headColor}  ◆  ÖZET${C.reset}`);
        console.log(`     ${C.bold}${scenarioTotal}${C.reset} senaryo  ${C.dim}(${C.reset}${breakdown}${C.dim})${C.reset}`);
